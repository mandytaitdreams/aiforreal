...
create type public.membership_tier as enum ('free', 'try', 'growth', 'power');
...
create policy "Users view own progress" on public.user_track_progress
  for select using (auth.uid() = user_id);
create policy "Users insert own progress" on public.user_track_progress
  for insert with check (auth.uid() = user_id);
create policy "Users update own progress" on public.user_track_progress
  for update using (auth.uid() = user_id);
